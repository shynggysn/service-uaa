...
public class CustomsRequestDeclaration implements ICustomsReqcuestDecarartion {
	...
	public SaveDeclarationResponseType send(String user, String password, String customsCode, String request) {
		...
		try {
			...
			response.setValue("Сервис временно недоступен. Пожалуйста, повторите запрос позднее.("+e.getLocalizedMessage()+")");
	...
	public SaveDeclarationResponseType send(String user, String password, String customsCode, PIRWInformationCUType request) throws Exception {
		return send(user, password, customsCode, objectToString(request));
	}

	public SaveDeclarationResponseType sendTD(String user, String password, String customsCode, ESADoutCUType request) throws Exception {
	...
	public String objectToStringTD(ESADoutCUType request) {
		...
		try {
			...
			ru.customs.information.customsdocuments.esadout_cu._5_11.ObjectFactory ob = new ru.customs.information.customsdocuments.esadout_cu._5_11.ObjectFactory();
			StringWriter sw = new StringWriter();
			marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
			marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
			marshaller.setProperty("com.sun.xml.bind.namespacePrefixMapper", new PINamespacePrefixMapper());
