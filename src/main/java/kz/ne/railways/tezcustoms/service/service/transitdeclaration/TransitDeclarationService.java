...
public class TransitDeclarationService implements TransitDeclarationServiceLocal {
	...
	public void send(String invoiceId_s, String name, String surname) {
		SaveDeclarationResponseType result = null;

		Long invoiceId = Long.parseLong(invoiceId_s);

		//        if (user != null) {
		TransitDeclarationService.User userTD = new TransitDeclarationService.User(name, surname);
		setUser(userTD);
//        }
		setDao(dao);
		setSender(sender);

		ESADoutCUType tdDoc = build(invoiceId);
		if(tdDoc != null) {
			//	System.out.println( td.getXml(tdDoc) );
			result = sendTD(invoiceId);
		}
		if (result != null && result.getValue() != null) {
			...
			result.setValue(str);
			Integer status = "0".equals(result.getCode().trim()) ? ForDataBean.PI_STATUS_SUCCESS_SEND : ForDataBean.PI_STATUS_FAIL_SEND;

			System.out.println(" * * * * * * before invoiceReq 222 invoiceid = "+ invoiceId + " status = " + status);
			dataBean.saveCustomsResponse(invoiceId, result, getUiid());

			if (status == 1)
				//TDG-4763 При наличии отправки, созданной на основании сообщения  IFTMIN, не создавать повторно вторую отправку
				// на основе переданного предварительно информирования в модуле "Предварительное информирование".
				if (dao.existLikeInvoiceByNumAndSta(invoiceId)==0) {
					System.out.println(" * * * * * * invoiceReq 222 ");

					NeInvcRefPi neInvcRefPi = dao.getNeInvcRefPi(invoiceId);
					if (neInvcRefPi == null) {
						invoiceReq(invoiceId, user == null ? "unknown" : user.getName());
					} else {
						NeInvoicePrevInfo invoicePrevInfo = dao.getInvoicePrevInfo(invoiceId);
						invoicePrevInfo.setPrevInfoStatus(1);
						dao.updatePrevInfoStatus(invoicePrevInfo);
					}
				}
		}
	...
	private ESADoutCUType createESADoutCUType() {
		...
		result.setDocumentModeID(getDocumentMode()); //Идентификатор вида документа. (Код документа в Альбоме форматов для РФ)
		...
		result.setCustomsModeCode(getCustomsModeCode()); // Код таможенной процедуры в соответствии с классификатором видов таможенных процедур гр. 1. второй подраздел ДТ. Первый элемент первого подраздела гр. 37/«00» – для припасов
		result.setTransitFeature(getTransitFeature()); // Особенность помещения товаров под процедуру таможенного транзита. гр. 1. второй подраздел ТД. МПО - международные почтовые отправления, ФЛ - товары и (или) транспортные средства для личного пользования*
		result.setTransitDirectionCode(getTransitDirectionCode()); // ИМ, ЭК, ТР, ВТ, ТС. Гр. 1 ТД. Третий подраздел
		result.setDeclarationKind(getDeclrationKind()); //Код особенности декларирования товаров по классификатору особенностей таможенного декларирования товаров. Гр.7 ДТ
		...
		result.setRecipientCountryCode(COUNTRY_CODE_2_ALPHA); // Код страны, в сооветствии с классификатором стран мира, таможенного органа, которому представляется документ
		...
		result.setRegNumberDoc(null); // Исходящий номер регистрации документов в соответствии с системой (регламентом) учета исходящих документов декларанта или таможенного представителя в Республике Беларусь
	...
	private String getDeclrationKind() {
		// TODO Auto-generated method stub
		// Код особенности декларирования товаров по классификатору особенностей таможенного декларирования товаров. Гр.7 ДТ
		return null;
	}

	private String getCustomsModeCode() {
		// TODO Auto-generated method stub
		// Код таможенной процедуры в соответствии с классификатором видов таможенных процедур гр. 1. второй подраздел ДТ. Первый элемент первого подраздела гр. 37/«00» – для припасов
		String result = null;
		return result;
	}
	...
	private ESADFilledPersonType getFilledPerson() {
		...
		result.setAuthoritesDocument(document); // Сведения о документе, удостоверяющем полномочия, о доверенности на совершение действий
		...
		result.setQualificationCertificate(null); // Номер квалификационного аттестата специалиста по таможенному оформлению. Для РБ
		return result;
	}
	...
	private ESADoutCUGoodsShipmentType getGoodsShipment() {
		...
		result.setOriginCountryName(getOriginCountryName()); // Страна происхождения товара. Краткое название страны в соответствии с классификатором стран мира / РАЗНЫЕ/ НЕИЗВЕСТНА/ ЕВРОСОЮЗ
		result.setESADoutCUConsignor(getConsignor());// "Отправитель/Экспортер"
		result.setESADoutCUConsignee(getConsignee()); // "Получатель"
		result.setESADoutCUFinancialAdjustingResponsiblePerson(getFinancialAdjustingResponsiblePerson()); // Лицо ответственное за финансовое урегулирование
		...
		result.setESADoutCUConsigment(getConsigment()); // Сведения о перевозке товаров. Гр. 15, 15а, 17,17а, 18, 19, 21, 25, 26, 29 -ДТ/ Гр. 15, 17, 18, 19, 21, 25, 26, 29 , 53, 55- ТД
		result.setESADoutCUPayments(getPayments()); //Сведения об уплачиваемых платежах, платежных поручениях, отсрочке платежей. Гр. 48, В
		...
		result.setSpecificationListNumber(getSpecificationListNumber(result)); // Общее количество листов представленных спецификаций
		...
		result.setCustCostCurrencyCode(null); // Буквенный код валюты таможенной стоимости в соответствии с классификатором валют
	...
	private ESADoutCUMainContractTermsType getMainContractTerms(ESADoutCUGoodsShipmentType goodsShipemnt) {
		...
		result.setContractCurrencyCode(currencyCode); // Трехзначный буквенный в графе 22 код валюты цены договора/ платежа (оценки). По классификатору валют
		result.setCurrencyQuantity(null); // Количество единиц валюты, для которой указан курс для РБ
		result.setContractCurrencyRate(getContractCurrencyRate(currencyCode)); // Курс валюты цены договора/ платежа (оценки)
		result.setTotalInvoiceAmount(getTotalInvoiceAmount()); // Общая стоимость товаров. Гр 22 подраздел 2
		result.setTradeCountryCode(null); // Буквенный код торгующей страны в соответствии с классификатором стран мира. Гр.11, 1-й подраздел ДТ
		result.setRBCountryCode(null); //Код административно-территориального деления страны в соответствии с классификатором административно-территориального деления стран, применяемым в Республике Беларусь. Гр.11, 2-й подраздел ДТ
		result.setDealFeatureCode(null); // Код особенности сделки (договора). В соответствии с классификатором особенностей внешнеэкономической сделки, используемым в государствах - членахТаможенного союзаЕвразийского экономического союза. Гр. 24 ДТ
		result.setDealNatureCode(null); // Код характера сделки с декларируемыми товарами в соответствии с классификатором характера сделки, используемым в государствах - членахТаможенного союзаЕвразийского экономического союза. Гр.24 ДТ
	...
	private BigDecimal getContractCurrencyRate(String currencyCode) {
		...
		if ("KZT".equals(currencyCode)) {
			...
			if (rates != null)
				return rates.get(currencyCode) != null ? rates.get(currencyCode).setScale(2, BigDecimal.ROUND_HALF_UP) : null;
			else
				return BigDecimal.ONE;
		}
	}
	...
	private String getCurrencyCode() {
		// Трехзначный буквенный в графе 22 код валюты цены договора/ платежа (оценки). По классификатору валют
		String result = "";
		if (tnVedList != null)
		for(TnVedRow row:tnVedList) {
	...
	private void buildGoods(List<ESADoutCUGoodsType> doutCUGoods) {
		...
		if (tnVedList != null && !tnVedList.isEmpty()) {
			...
			for (TnVedRow tnVedRow : tnVedList) {
				...
				//в ХМL для товаров с одинаковыми кода ТН ВЭД, но разными видами упаковки сведения передавать отдельными группами
				if (goods == null || !tnVedRow.getPackingCode().equals(goods.getESADGoodsPackaging().getPackageCode())) {
				...
				//--
				goods.setAdditionalSheetCount(getAdditionalSheetCount(i++)); //Порядковый номер листа (первый подраздел гр.3)

				Integer containerCount = goods.getESADContainer().getContainerNumber().size();
				goods.getESADContainer().setContainerQuantity(containerCount > 0 ? BigInteger.valueOf(containerCount) : null);
				goods.getESADContainer().setContainerKind(getContainerKind()); // Тип контейнера в соответствии с классификатором видов груза, упаковки и упаковочных материалов
			...
			//for(String key: goodsMap.keySet()) {
				...
				ESADoutCUGoodsType goods = goodsMap.get(tnVedRow.getTnVedCode()+tnVedRow.getPackingCode()+tnVedRow.getId());
	...
	private ESADoutCUGoodsType addGoods(ESADoutCUGoodsType goods, TnVedRow tnVedRow) {
		goods.setGrossWeightQuantity(goods.getGrossWeightQuantity().add(tnVedRow.getBrutto() != null ? tnVedRow.getBrutto() : BigDecimal.ZERO).setScale(3, BigDecimal.ROUND_HALF_UP));
		goods.setInvoicedCost(goods.getInvoicedCost().add(tnVedRow.getPriceByTotal() != null ? tnVedRow.getPriceByTotal(): BigDecimal.ZERO).setScale(2, BigDecimal.ROUND_HALF_UP));
		goods.setCustomsCost(null);
		goods.setNetWeightQuantity(goods.getNetWeightQuantity().add(tnVedRow.getNetto() != null ? tnVedRow.getNetto() : BigDecimal.ZERO).setScale(3, BigDecimal.ROUND_HALF_UP));
		...
		packagingType.setPakageQuantity(packagingType.getPakageQuantity().add(tnVedRow.getPlaceCargoCount() != null ? tnVedRow.getPlaceCargoCount() : BigDecimal.ONE).setScale(2, BigDecimal.ROUND_HALF_UP));
		if (packagingType.getPakagePartQuantity() != null) {
			packagingType.setPakagePartQuantity(packagingType.getPakagePartQuantity().add(tnVedRow.getPakagePartQuantity() != null ? tnVedRow.getPakagePartQuantity() : BigDecimal.ONE).setScale(2, BigDecimal.ROUND_HALF_UP));
		}
		...
		for (int i=0; i < sizePackType; i++) {
			PackingInformationType informationType = (PackingInformationType)packagingType.getPackingInformation().get(i);
			quantity = tnVedRow.getPackingCount() != null ? BigInteger.valueOf(tnVedRow.getPackingCount().longValue()+informationType.getPakingQuantity().longValue()) : BigInteger.ONE;
	...
	private ESADoutCUGoodsType buildGoods(TnVedRow tnVedRow) {
		...
		result.setGrossWeightQuantity(tnVedRow.getBrutto() != null ? tnVedRow.getBrutto().setScale(3, BigDecimal.ROUND_HALF_UP) : BigDecimal.ZERO.setScale(3, BigDecimal.ROUND_HALF_UP));
		result.setInvoicedCost(tnVedRow.getPriceByTotal() != null ? tnVedRow.getPriceByTotal().setScale(2, BigDecimal.ROUND_HALF_UP) : BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));
		result.setCustomsCost(null);
		result.setNetWeightQuantity(tnVedRow.getNetto() != null ? tnVedRow.getNetto().setScale(3, BigDecimal.ROUND_HALF_UP) : BigDecimal.ZERO.setScale(3, BigDecimal.ROUND_HALF_UP));
		...
		if (tnVedRow.getPakagePartQuantity() != null && tnVedRow.getPakagePartQuantity().toBigInteger() != BigInteger.ZERO ) {
			packagingType.setPakagePartQuantity(tnVedRow.getPakagePartQuantity() != null ? tnVedRow.getPakagePartQuantity().setScale(2, BigDecimal.ROUND_HALF_UP) :  BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));
		}
		//TAV возможно фикс по количеству грузомест
		//packagingType.setPakageQuantity(tnVedRow.getPackingCount() != null ? tnVedRow.getPackingCount().setScale(2, BigDecimal.ROUND_HALF_UP) :  BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));
		packagingType.setPakageQuantity(tnVedRow.getPlaceCargoCount() != null ? tnVedRow.getPlaceCargoCount().setScale(2, BigDecimal.ROUND_HALF_UP) :  BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));

		//Если вид упаковки: навалом(VS), насыпью(VO, VR, VY), наливом(VL, VQ), неупакован(NE, NF, NG) или нет сведений (NA), то передавать PakageTypeCode=0.
		String pkgCode = tnVedRow.getPackingCode();
		if (pkgCode != null && (pkgCode.equals("VS") || pkgCode.equals("VO") || pkgCode.equals("VR") ||
				pkgCode.equals("VY") || pkgCode.equals("VL") || pkgCode.equals("VQ") ||
				pkgCode.equals("NE") || pkgCode.equals("NF") || pkgCode.equals("NG") || pkgCode.equals("NA")))
			packagingType.setPakageTypeCode("0");
		else
			packagingType.setPakageTypeCode((pkgCode != null ? "1" : "2")); // Если есть код упаковки, то ставим "С упаковкой", иначе "Без упаковки в оборудованных емкостях транспортного средства"

		PackingInformationType goodsPackingInformationType = new PackingInformationType();
		goodsPackingInformationType.setPackingCode(tnVedRow.getPackingCode());
		goodsPackingInformationType.setPakingQuantity(tnVedRow.getPackingCount() != null ? BigInteger.valueOf(tnVedRow.getPackingCount().longValue()) : null);
	...
	private void addContainer(ESADoutCUGoodsType goods, TnVedRow tnVedRow) {
		...
		if(!containerList.isEmpty() && !containers.isEmpty() && invoice.getIsContainer() == 1) {// По новой логике
			for(String key: containers.keySet()) {
				...
				if (!haveContainer(goodsContainer.getContainerNumber(), container)) { // Проверяем был ли добавлен уже такой контейнер
			...
			for (String item : containersTnVed) {
				...
				if (!StringUtils.isEmpty(itemTrim) && !containsContiner(goodsContainer.getContainerNumber(), itemTrim)) {
					ContainerNumberType container = new ContainerNumberType();
	...
	private boolean haveContainer(List<ContainerNumberType> containerNumber, ContainerNumberType container) {
		if (!containerNumber.isEmpty() && container != null) {
			for(ContainerNumberType item: containerNumber) {
				if (item.getContainerIdentificaror() != null
						&& container.getContainerIdentificaror() != null
...
//			int limit = 250;
			...
			if (doc.getDocumentName() != null)
				string = doc.getDocumentName().length() > limit ? doc.getDocumentName().substring(0, limit) : doc.getDocumentName();
	...
	private ESADoutCUConsigmentType getConsigment() {
		...
		if (invoicePrevInfo == null) return result;

		result.setContainerIndicator(invoice.getIsContainer() == 1 ? true	: false); // Признак контейнерных перевозок. Гр.19
		Country dispatchCountry = invoicePrevInfo != null ? dao.getCountry(invoicePrevInfo.getStartStaCouNo()) :  null;
		result.setDispatchCountryCode(dispatchCountry != null ? dispatchCountry.getCountryId() :null); // Страна отправления. Буквенный код страны в соответствии с классификатором стран мира. Гр 15, подраздел a
		result.setDispatchCountryName(dispatchCountry != null ? dispatchCountry.getCountryName() :null); // Страна отправления. Краткое название страны в соответствии с классификатором стран мира. Гр.15
		result.setRBDispatchCountryCode(null); // Код административно-территориального деления страны отправления в соответствии с классификатором административно-территориального деления стран, применяемым в Республике Беларусь. Гр.15, подраздел b
		Country destinationCountry = invoicePrevInfo != null ? dao.getCountry(invoicePrevInfo.getDestStationCouNo()) :  null;
		result.setDestinationCountryCode(destinationCountry != null ? destinationCountry.getCountryId() : null); // Страна назначения. Буквенный код страны в соответствии с классификатором стран мира / 00 (неизвестна). Гр 17, подраздел a
		result.setDestinationCountryName(destinationCountry != null ? destinationCountry.getCountryName() : null); // Страна назначения. Краткое название страны в соответствии с классификатором стран мира / НЕИЗВЕСТНА. Гр 17
		result.setRBDestinationCountryCode(null); // Код административно-территориального деления страны назначения в соответствии с классификатором административно-территориального деления стран, применяемым в Республике Беларусь. Гр.17, подраздел b
		result.setDateExpectedArrival(convertDate(invoicePrevInfo.getArriveTime())); // Дата ожидаемого прибытия товаров и транспортных средств в пограничный пункт пропуска. ТД
		result.setTimeExpectedArrival(convertTime(invoicePrevInfo.getArriveTime())); // Время ожидаемого прибытия товаров и транспортных средств в пограничный пункт пропуска. ТД

		result.setESADoutCUDepartureArrivalTransport(getDepartureArrivalTransport()); // Транспортные средства при прибытии/убытии. Гр. 18, 26

		result.setESADoutCUBorderTransport(getDepartureBorderTransportType()); // Транспортные средства на границе. Гр. 21, 25 ДТ
		// old 5.10
		//result.setDeliveryCustomsOffice(getDeliveryCustomsOffice()); // Таможенный орган назначения при транзите.гр.53 ТД
		// new 5.11
		buildTDDeliveryPlace(result);

		result.setPPBorderCustoms(getPPBorderCustoms()); // Таможенный орган (пограничный пункт пропуска), в который ожидается прибытие товаров и транспортных средств
	...
	private ESADoutCUDepartureArrivalTransportType getDepartureArrivalTransport() {
		...
		result.setTransportNationalityCode(getTransportNationalityCode()); // Буквенный код страны принадлежности (регистрации) транспортных средств по классификатору стран мира. 99-разные, 00-неизвестна
		result.setTransportMeansQuantity(getTransportMeansQuantity(true)); // Количество транспортных средств
		result.setMethodTransport(null); // Способ транспортировки товаров при перемещении по линиям электропередачи или трубопроводным транспортом: 1 - газопровод; 2 - нефтепровод; 3 - нефтепродуктопровод; 4 - линии электропередачи
		result.setNameObject(null); // Наименование объекта, на котором установлены приборы учета товаров, перемещаемых по линиям электропередачи или трубопроводным транспортом
	...
	private BigInteger getTransportMeansQuantity(boolean isArrival) {
		...
		int count = 0;
		if(invoice.getIsContainer()!=1 && vagonLists!=null && !vagonLists.isEmpty() && vagonLists.size()>1 && mapVagons!=null){
	...
	private String getTransportNationalityCode() {
		// Буквенный код страны принадлежности (регистрации) транспортных средств по классификатору стран мира. 99-разные, 00-неизвестна
		return null;
/*		String result = null;
		for(NeVagonLists vagon: vagonLists) {
			String item = getTransportMeansNationalityCode(vagon.getOwnerRailways());
			if(result == null) {
				if (item != null) {
					result = item;
				}
...
*/	}
	...
	private boolean buildTransportMeans(List<TransportMeansBaseType> transportMeans,boolean isArrival) {
		...
		boolean isChine = Arrays.asList("708507","707608","708403","707701","710102").contains(invoicePrevInfo.getArriveStaNo());
			...
			if (isArrival) {//18
				...
				if (has7Length) {
					for (NeContainerLists container : containerList) {
						...
						transportMeansBaseType.setTransportIdentifier(container.getContainerMark() + container.getContainerNo()); //Идентификатор. Номер транспортного средства, наименование судна, номер авиарейса, номер поезда, номер железнодорожного вагона (платформ, цистерн и т.п.)
						//transportMeansBaseType.setTransportMeansNationalityCode(getTransportMeansNationalityCode(container.getManagUn())); // Буквенный код страны принадлежности транспортного средства по классификатору стран мира
						transportMeansBaseType.setTransportRegNumber(null); // Номер свидетельства о регистрации транспортного средства для РБ
					...
					for(NeVagonLists vagon: vagonLists) {
						...
						transportMeansBaseType.setTransportKindCode("20"); // Код типа транспортного средства
						transportMeansBaseType.setTransportMarkCode(null); // Код марки транспортного средства
						transportMeansBaseType.setTransportIdentifier(vagon.getVagNo()); //Идентификатор. Номер транспортного средства, наименование судна, номер авиарейса, номер поезда, номер железнодорожного вагона (платформ, цистерн и т.п.)
						//transportMeansBaseType.setTransportMeansNationalityCode(getTransportMeansNationalityCode(vagon.getOwnerRailways())); // Буквенный код страны принадлежности транспортного средства по классификатору стран мира
						transportMeansBaseType.setTransportRegNumber(null); // Номер свидетельства о регистрации транспортного средства для РБ
						transportMeans.add(transportMeansBaseType);
					}
				}
				...
				if (has7Length && vagonLists != null && vagonLists.size() > 0) {
					...
					if(ves != null) {
						...
						transportMeansBaseType.setTransportMeansNationalityCode("AZ"); // Буквенный код страны принадлежности транспортного средства по классификатору стран мира
					}
					//Идентификатор. Номер транспортного средства, наименование судна, номер авиарейса, номер поезда, номер железнодорожного вагона (платформ, цистерн и т.п.)
...
//							transportMeansBaseType.setTransportKindCode("20"); // Код типа транспортного средства
//							transportMeansBaseType.setTransportMarkCode(null); // Код марки транспортного средства
//							transportMeansBaseType.setTransportIdentifier(item); //Идентификатор. Номер транспортного средства, наименование судна, номер авиарейса, номер поезда, номер железнодорожного вагона (платформ, цистерн и т.п.)
//							for(NeVagonLists vagon: vagonLists)
//								transportMeansBaseType.setTransportMeansNationalityCode(getTransportMeansNationalityCode(vagon.getOwnerRailways())); // Буквенный код страны принадлежности транспортного средства по классификатору стран мира
//							transportMeansBaseType.setTransportRegNumber(null); // Номер свидетельства о регистрации транспортного средства для РБ
//							transportMeans.add(transportMeansBaseType);
...
//			}
			...
			if(!isArrival && isParom) {
				...
				NeVessel ves = dao.getVessel(ship.getNeVesselUn());
				if(ves != null) {
					transportMeansBaseType.setTransportIdentifier(ves.getVesselName());//Идентификатор. Номер транспортного средства, наименование судна, номер авиарейса, номер поезда, номер железнодорожного вагона (платформ, цистерн и т.п.)
					transportMeansBaseType.setTransportMeansNationalityCode("AZ"); // Буквенный код страны принадлежности транспортного средства по классификатору стран мира
				}
...
//		itnkz.setRNN(ktzh.getLPersRnn());
		if (!StringUtils.isEmpty(itnkz.getCategoryCode())
				|| !StringUtils.isEmpty(itnkz.getITNReserv())
				|| !StringUtils.isEmpty(itnkz.getKATOCode())
				|| !StringUtils.isEmpty(itnkz.getRNN())
				) {
	...
	private ESADoutCUDeclarantType getDeclarant() {
		...
		if (declarantInfo != null) {
			...
			result.setOrganizationName(replaceSpacSymbol(declarantInfo.getDeclarantName())); // Наименование организации / ФИО физического лица
			result.setShortName(replaceSpacSymbol(declarantInfo.getDeclarantShortName())); // Краткое наименование организации
			...
			if ("KZ".equals(country.getCountryId())) {
				...
				features.setIIN(!StringUtils.isEmpty(declarantInfo.getDeclarantKZIin()) ? declarantInfo.getDeclarantKZIin() : null); // Индивидуальный идентификационный номер (ИИН)
				ITNKZType itn = new ITNKZType();
				itn.setCategoryCode(declarantInfo.getDeclarantKZPersonsCategory()); // Категория лица. Двухзначный цифровой код категории лица согласно учредительным документам или свидетельству о регистрации в качестве индивидуального предпринимателя
				itn.setITNReserv(!StringUtils.isEmpty(declarantInfo.getDeclarantKZITN()) ? declarantInfo.getDeclarantKZITN() : null ); //Резерв для ИТН
				itn.setRNN(null); // РНН. Регистрационный налоговый номер, присваиваемый налоговыми органами Республики Казахстан
				itn.setKATOCode(declarantInfo.getDeclarantKZKATO()); // Код КАТО. Двухзначный код КАТО в соответствии с классификатором кодов административно-территориальных объектов
				if (!StringUtils.isEmpty(itn.getCategoryCode())
						|| !StringUtils.isEmpty(itn.getITNReserv())
						|| !StringUtils.isEmpty(itn.getKATOCode())
						|| !StringUtils.isEmpty(itn.getRNN())
						) {
				...
				result.setRFOrganizationFeatures(rfFeaturesType); // Сведения об организации. Особенности Российской Федерации
				...
				result.setRAOrganizationFeatures(raFeaturesType); // Сведения об организации. Особенности республики Армения
				...
				result.setRBOrganizationFeatures(rbFeaturesType); // Сведения об организации. Особенности Республики Беларусь
				...
				result.setKGOrganizationFeatures(kgFeaturesType); // Сведения об организации. Особенности Кыргызской Республики
			}
			AddressType address = new AddressType();
			if (country != null) {
				address.setCountryCode(country.getCountryId()); // Буквенный код страны в соответствии с классификатором стран мира
				address.setCounryName(country.getCountryName()); // Краткое название страны в соответствии с классификатором стран мира
	...
	private ESADoutCUDeclarantType getExpeditor() {
		...
		if (expeditorInfo != null) {
			...
			result.setOrganizationName(replaceSpacSymbol(expeditorInfo.getExpeditorName())); // Наименование организации / ФИО физического лица
			result.setShortName(replaceSpacSymbol(expeditorInfo.getExpeditorShortName())); // Краткое наименование организации
			result.setOrganizationLanguage(DOCUMENT_LANGUAGE); // Код языка для заполнения наименования
			if ("KZ".equals(country.getCountryId())) {
				RKOrganizationFeaturesType features = new RKOrganizationFeaturesType();
				features.setBIN(expeditorInfo.getExpeditorKZBin()); // Бизнес-идентификационный номер (БИН)
				features.setIIN(!StringUtils.isEmpty(expeditorInfo.getExpeditorKZIin()) ? expeditorInfo.getExpeditorKZIin() : null); // Индивидуальный идентификационный номер (ИИН)
				ITNKZType itn = new ITNKZType();
				itn.setCategoryCode(expeditorInfo.getExpeditorKZPersonsCategory()); // Категория лица. Двухзначный цифровой код категории лица согласно учредительным документам или свидетельству о регистрации в качестве индивидуального предпринимателя
				itn.setITNReserv(!StringUtils.isEmpty(expeditorInfo.getExpeditorKZITN()) ? expeditorInfo.getExpeditorKZITN() : null ); //Резерв для ИТН
				itn.setRNN(null); // РНН. Регистрационный налоговый номер, присваиваемый налоговыми органами Республики Казахстан
				itn.setKATOCode(expeditorInfo.getExpeditorKZKATO()); // Код КАТО. Двухзначный код КАТО в соответствии с классификатором кодов административно-территориальных объектов
				if (!StringUtils.isEmpty(itn.getCategoryCode())
						|| !StringUtils.isEmpty(itn.getITNReserv())
						|| !StringUtils.isEmpty(itn.getKATOCode())
						|| !StringUtils.isEmpty(itn.getRNN())
	...
	private ESADoutCUFinancialAdjustingResponsiblePersonType getFinancialAdjustingResponsiblePerson() {
		...
		ESADoutCUFinancialAdjustingResponsiblePersonType  result = new ESADoutCUFinancialAdjustingResponsiblePersonType();
		return result;
	}
	...
	private ESADoutCUConsigneeType getConsignee() {
		...
		if (recieverInfo != null) {
			...
			result.setOrganizationName(recieverInfo.getRecieverFullName()!= null ? replaceSpacSymbol(recieverInfo.getRecieverFullName()) : null); // Наименование организации / ФИО физического лица
			result.setShortName(recieverInfo.getRecieverName() != null ? replaceSpacSymbol(recieverInfo.getRecieverName()) : null); // Краткое наименование организации
			result.setOrganizationLanguage(DOCUMENT_LANGUAGE); // Код языка для заполнения наименования
			if (country != null) {
				if ("KZ".equals(country.getCountryId())) {
					RKOrganizationFeaturesType features = new RKOrganizationFeaturesType();
					features.setBIN(recieverInfo.getRecieverBin()); // Бизнес-идентификационный номер (БИН)
					features.setIIN(!StringUtils.isEmpty(recieverInfo.getRecieverIin()) ? recieverInfo.getRecieverIin() : null); // Индивидуальный идентификационный номер (ИИН)
					ITNKZType itn = new ITNKZType();
					itn.setCategoryCode(getCategoryCodeReciever(recieverInfo.getCategoryType())); // Категория лица. Двухзначный цифровой код категории лица согласно учредительным документам или свидетельству о регистрации в качестве индивидуального предпринимателя
					itn.setITNReserv(!StringUtils.isEmpty(recieverInfo.getItn()) ? recieverInfo.getItn() : null); //Резерв для ИТН
					itn.setRNN(null); // РНН. Регистрационный налоговый номер, присваиваемый налоговыми органами Республики Казахстан
					itn.setKATOCode(recieverInfo.getKatoType() != null ? recieverInfo.getKatoType().toString() : null); // Код КАТО. Двухзначный код КАТО в соответствии с классификатором кодов административно-территориальных объектов
					if (!StringUtils.isEmpty(itn.getCategoryCode())
							|| !StringUtils.isEmpty(itn.getITNReserv())
							|| !StringUtils.isEmpty(itn.getKATOCode())
							|| !StringUtils.isEmpty(itn.getRNN())
			...
			if (country != null) {
				...
				address.setPostalCode(!StringUtils.isEmpty(recieverInfo.getRecieverPostIndex()) ? recieverInfo.getRecieverPostIndex() : "000000" ); // Почтовый индекс
				address.setPostalCode("000000" ); // Почтовый индекс
				address.setTerritoryCode(null); // Код единицы административно-территориального деления
			}
			result.setAddress(address); // Адрес организации
	...
	private ESADoutCUConsignorType getConsignor() {
		...
		if (senderInfo != null) {
			...
			result.setOrganizationName(replaceSpacSymbol(senderInfo.getSenderFullName())); // Наименование организации / ФИО физического лица
			result.setShortName(senderInfo.getSenderName() == null ? null : senderInfo.getSenderName().replaceAll("&", "and")); // Краткое наименование организации
			result.setOrganizationLanguage(DOCUMENT_LANGUAGE); // Код языка для заполнения наименования
			if (country != null) {
				if ("KZ".equals(country.getCountryId())) {
					RKOrganizationFeaturesType features = new RKOrganizationFeaturesType();
					features.setBIN(senderInfo.getSenderBin()); // Бизнес-идентификационный номер (БИН)
					features.setIIN(!StringUtils.isEmpty(senderInfo.getSenderIin()) ? senderInfo.getSenderIin() : null ); // Индивидуальный идентификационный номер (ИИН)
					ITNKZType itn = new ITNKZType();
					itn.setCategoryCode(getCategoryCode(senderInfo.getCategoryType())); // Категория лица. Двухзначный цифровой код категории лица согласно учредительным документам или свидетельству о регистрации в качестве индивидуального предпринимателя
					itn.setITNReserv(!StringUtils.isEmpty(senderInfo.getItn()) ? senderInfo.getItn() : null); //Резерв для ИТН
					itn.setRNN(null); // РНН. Регистрационный налоговый номер, присваиваемый налоговыми органами Республики Казахстан
					itn.setKATOCode(senderInfo.getKatoType() != null ? senderInfo.getKatoType().toString() : null); // Код КАТО. Двухзначный код КАТО в соответствии с классификатором кодов административно-территориальных объектов
					if (!StringUtils.isEmpty(itn.getCategoryCode())
							|| !StringUtils.isEmpty(itn.getITNReserv())
							|| !StringUtils.isEmpty(itn.getKATOCode())
							|| !StringUtils.isEmpty(itn.getRNN())
			...
			if (country != null) {
				...
				//address.setPostalCode(!StringUtils.isEmpty(senderInfo.getSenderPostIndex()) ? senderInfo.getSenderPostIndex() : "000000"); // Почтовый индекс
				address.setPostalCode("000000"); // Почтовый индекс
				address.setTerritoryCode(null); // Код единицы административно-территориального деления
			}
			result.setAddress(address); // Адрес организации
	...
	private String getTransitFeature() {
		// Особенность помещения товаров под процедуру таможенного транзита. гр. 1. второй подраздел ТД. МПО - международные почтовые отправления, ФЛ - товары и (или) транспортные средства для личного пользования*
		String result = null;
...
/*		if ("1".equals(invoicePrevInfo.getPrevInfoType()) ) {
			result = CustomsProcedure.TT;
		} else if ("2".equals(invoicePrevInfo.getPrevInfoType())) {
			result = CustomsProcedure.IM;
		} else {
			result = CustomsProcedure.EK;
		}
*/
	...
	private void initContainers() {
		...
		if (invoice.getIsContainer() == 1 && !containerList.isEmpty()) { // Если у нас контейнерная отправка и есть данные в NE_CONTAINERS_LIST
			for(TnVedRow tnVedRow : tnVedList) {
				...
				for (NeContainerLists item : containerList) {
					String itemTrim = ((item.getContainerMark()!=null ? item.getContainerMark() : "") + (item.getContainerNo()!=null ? item.getContainerNo() : "")).trim();
					if (!StringUtils.isEmpty(itemTrim)) {
	...
	/*private SaveDeclarationResponseType initReplaceTnVedCode() {
		SaveDeclarationResponseType result = null;
		replaceTnVedCode = new HashMap<String, String>();
		if (sender != null) {
			PIRWInformationCUType sendData = makeData(invoiceUn);
			this.uuid = sendData.getUINP();

			int attempt = 0; // пытаемся обрезать код ТН ВЭД до 6 символов!
			boolean ex = true;
			while (ex) {
				ex = false;
				result = sender.send(sendData);
				if (isGngError(result) && attempt < 3) { //3 попытки
					attempt++;
					sendData = changeGng(result, sendData);
					ex = true;
				} else {
					// Ошибка ФЛК 301 со стороны Т.
					if (attempt != 0) {
						String code = result.getCode();
						String message = "КОД ОШИБКИ - " + code;
						try {
							message += getErrValueMess(result, sendData);
							if (!code.equals("0"))
								result.setValue(message + " " + result.getValue());
						} catch(Exception exn) {
							exn.printStackTrace();
						}
					}
			...
			if (!splitList.isEmpty()) {
				...
				for (PICUGoodsShipmentType goodsShipment: sendData.getPICUGoodsShipment()) {
					for(PIRWGoodsType goods: goodsShipment.getPIGoods()) {
						...
						if (goods.getGoodsNumeric() != null && goodsNumber.contains(goods.getGoodsNumeric().intValue())) {
							if (goods.getGoodsTNVEDCode() != null && goods.getGoodsTNVEDCode().length() > 6) {
								//String code = goods.getGoodsTNVEDCode().substring(0, goods.getGoodsTNVEDCode().length() - 4);
								String code = goods.getGoodsTNVEDCode().substring(0, 6); // Режим до 6 символов

								log.info(goods.getGoodsNumeric().intValue() +": "+ goods.getGoodsTNVEDCode() + " --> "+ code);
		...
		if (result != null && result.getValue() != null && sendData != null) {
			...
			if (!splitList.isEmpty()) {
				...
				for (PICUGoodsShipmentType goodsShipment: sendData.getPICUGoodsShipment()) {
					for(PIRWGoodsType goods: goodsShipment.getPIGoods()) {
						if (goods.getGoodsNumeric() != null && goodsNumber.contains(goods.getGoodsNumeric().intValue())) {
							message += " ( Индекс: " +goods.getGoodsNumeric().toString()+ ", Код ТНВЭД: " +goods.getGoodsTNVEDCode()+ " ); ";
						}
					}
				}
			}
	...
	public enum TransitDirectionCode { // Особенность помещения товаров под процедуру таможенного транзита. гр. 1. второй подраздел ТД. МПО - международные почтовые отправления, ФЛ - товары и (или) транспортные средства для личного пользования*
		TR(1, "ТР"),
		IM(2, "ИМ"),
		EK(3, "ЭК"),
		VT(4, "ВТ"),
		...
		private static final Map<Integer, TransitDirectionCode> lookupByCode = new HashMap<Integer, TransitDirectionCode>(5);
