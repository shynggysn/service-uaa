...
public class PIMessageBuilder implements PIMessageBuilderLocal {
	...
	public void setNeSmgsDestinationPlaceInfo(
	...
	private PIRWInformationCUType buildInformationCUType() {
		...
		informationCUType.setDocumentModeID((Double.valueOf(Math.random()))
				.toString());
	...
	private PICUGoodsShipmentType buildGoodItem() {
		...
		if (senderInfo != null && recieverInfo != null) {
			...
			Country countryStart = (invoicePrevInfo != null ? dao.getCountry(invoicePrevInfo.getStartStaCouNo()) :  null);
			consignmentType.setDispatchCountryCode(countryStart != null ? countryStart.getCountryId() : country.getCountryId());
			consignmentType.setDispatchCountryName(countryStart != null ? countryStart.getCountryFullName() : country.getCountryFullName());
			...
			Country countryDest = (invoicePrevInfo != null ? dao.getCountry(invoicePrevInfo.getDestStationCouNo()) : null );
			consignmentType.setDestinationCountryCode(countryDest != null ? countryDest.getCountryId() : country.getCountryId());
			consignmentType.setDestinationCountryName(countryDest != null ? countryDest.getCountryFullName() : country.getCountryFullName());
			...
			if ("KZ".equals(country.getCountryId())) {
				rkFeaturesType = new RKOrganizationFeaturesType();
				rkFeaturesType = new RKOrganizationFeaturesType();
				rkFeaturesType.setBIN(StringUtils.isEmpty(recieverInfo.getRecieverBin()) ? null : recieverInfo.getRecieverBin());
				rkFeaturesType.setIIN(StringUtils.isEmpty(recieverInfo.getRecieverIin()) ? null : recieverInfo.getRecieverIin());
				itnkz = new ITNKZType();
			...
			if (invoicePrevInfo != null) {
				...
				if (arriveSta != null) {
					...
					customStation.setRailwayCode(managementArriveSta != null ? managementArriveSta.getMNameLat() : null);
				}
			}
	...
	private PIRWGoodsType addGoods(PIRWGoodsType result, TnVedRow tnVedRow) {
		...
		packagingType.setPakageQuantity(packagingType.getPakageQuantity().add(tnVedRow.getPackingCount() != null ? tnVedRow.getPackingCount() : BigDecimal.ZERO));
		packagingType.setPakagePartQuantity(packagingType.getPakagePartQuantity().add(tnVedRow.getPakagePartQuantity() != null ? tnVedRow.getPakagePartQuantity() : BigDecimal.ZERO));
	...
	private void addContainer(PIRWGoodsType result, TnVedRow tnVedRow) {
		...
		if(!containerList.isEmpty() && !containers.isEmpty() && invoice.getIsContainer() == 1) {// По новой логике
			for(String key: containers.keySet()) {
				...
				if (!haveContainer(result.getContainerNumber(), container)) { // Проверяем был ли добавлен уже такой контейнер
	...
	private CUOrganizationType buildDeclarant() {
		if (declarantInfo != null) {
			...
			Country country = dao.getCountry(declarantInfo
					.getDeclarantCountry());
			if (country != null) {
			...
			if ("KZ".equals(country.getCountryId())) {
				...
				itnkz = new ITNKZType();
				itnkz.setCategoryCode(declarantInfo
						.getDeclarantKZPersonsCategory());
				...
				raFeaturesType = new RAOrganizationFeaturesType();
				raFeaturesType.setSocialServiceNumber(declarantInfo
						.getDeclarantAMNZOU());
				raFeaturesType.setUNN(declarantInfo.getDeclarantAMUNN());
				result.setRAOrganizationFeatures(raFeaturesType);
			} else if ("BY".equals(country.getCountryId())) {
				rbFeaturesType = new RBOrganizationFeaturesType();
				rbFeaturesType.setRBIdentificationNumber(declarantInfo
						.getDeclarantBYIN());
	...
	private CUOrganizationType buildExpeditor() {
		if (expeditorInfo != null) {
			...
			Country country = dao.getCountry(expeditorInfo
					.getExpeditorCountry());
			if (country != null) {
				addressType.setCounryName(country.getCountryFullName());
				addressType.setCountryCode(country.getCountryId());
			...
			if ("KZ".equals(country.getCountryId())) {
				...
				itnkz = new ITNKZType();
				itnkz.setCategoryCode(expeditorInfo
						.getExpeditorKZPersonsCategory());
				...
				raFeaturesType = new RAOrganizationFeaturesType();
				raFeaturesType.setSocialServiceNumber(expeditorInfo
						.getExpeditorAMNZOU());
				raFeaturesType.setUNN(expeditorInfo.getExpeditorAMUNN());
				result.setRAOrganizationFeatures(raFeaturesType);
			} else if ("BY".equals(country.getCountryId())) {
				rbFeaturesType = new RBOrganizationFeaturesType();
				rbFeaturesType.setRBIdentificationNumber(expeditorInfo
						.getExpeditorBYIN());
